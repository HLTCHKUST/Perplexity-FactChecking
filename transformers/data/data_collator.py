...
class DataCollatorForLanguageModeling:
    ...
    def __call__(
        ...
        if self.mlm:
            ...
            mask_single_token = False
            mask_one_random = True

            assert sum([mask_single_token, mask_one_random]) == 1

            if mask_single_token:
                # labels[0][:-2]=-100 # mask all but last one + end-of-token
                # labels[0][-1]=-100
                # print(labels)

                inputs[0][-2] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)

                batch = {
                    "input_ids": _collate_batch(inputs, self.tokenizer),
                    "labels": _collate_batch(labels, self.tokenizer)
                }
            elif mask_one_random: # OG
                ...
                batch = {"input_ids": _collate_batch(inputs, self.tokenizer)}
                batch["input_ids"], batch["labels"] = self.mask_tokens(
